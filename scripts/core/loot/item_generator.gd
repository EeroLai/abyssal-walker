...
static func _pick_weighted_tier_index(tiers: Array, available_indices: Array[int]) -> int:
	...
	for idx in available_indices:
		...
		var weight: float = maxf(float(tier_num), 1.0)
...
static func _roll_rarity(floor_level: int) -> StatTypes.Rarity:
	var item_level: int = _floor_to_item_level(floor_level)
	var roll := randf()
...
static func _get_affix_count(rarity: StatTypes.Rarity, _item_level: int = 1) -> int:
	match rarity:
